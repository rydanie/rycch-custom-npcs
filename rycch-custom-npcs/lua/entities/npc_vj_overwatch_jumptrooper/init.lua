...
ENT.Model = {"models/missing_super_soldier.mdl"}
ENT.StartHealth = 60
...
ENT.MaxJumpLegalDistance = VJ_Set(1500, 1500)
	-- ====== Sound File Paths ====== --
...
function ENT:CustomOnInitialize()
    ...
    self.PackProp1 = ents.Create("prop_dynamic")
    self.PackProp1:SetModel("models/Items/AR2_Grenade.mdl")
    self.PackProp1:SetPos(self:GetPos() + self:OBBCenter() - (self:GetForward()*8+self:GetRight()*5+self:GetUp()*-15))
    self.PackProp1:SetAngles(self:GetAngles() + Angle(-90,0,0))
    self.PackProp1:SetParent(self,2)
    self.PackProp1:SetModelScale( self:GetModelScale() * 3, 0 )
    self.PackProp1:SetMaterial("models/props_combine/metal_combinebridge001")
    self.PackProp1:Spawn()
    self.PackProp1:AddEFlags(EFL_DONTBLOCKLOS)

    self.PackProp2 = ents.Create("prop_dynamic")
    self.PackProp2:SetModel("models/Items/AR2_Grenade.mdl")
    self.PackProp2:SetPos(self:GetPos() + self:OBBCenter() - (self:GetForward()*8+self:GetRight()*-5+self:GetUp()*-15))
    self.PackProp2:SetAngles(self:GetAngles() + Angle(-90,0,0))
    self.PackProp2:SetParent(self,2)
    self.PackProp2:SetModelScale( self:GetModelScale() * 3, 0 )
    self.PackProp2:SetMaterial("models/props_combine/metal_combinebridge001")
    self.PackProp2:Spawn()
    self.PackProp2:AddEFlags(EFL_DONTBLOCKLOS)

    self.PackProp3 = ents.Create("prop_dynamic")
    self.PackProp3:SetModel("models/props_combine/headcrabcannister01a.mdl")
    self.PackProp3:SetPos(self:GetPos() + self:OBBCenter() - (self:GetForward()*8+self:GetUp()*-15))
    self.PackProp3:SetAngles(self:GetAngles() + Angle(-90,0,0))
    self.PackProp3:SetParent(self,2)
    self.PackProp3:SetModelScale( self:GetModelScale() * .23, 0 )
    self.PackProp3:Spawn()
    self.PackProp3:AddEFlags(EFL_DONTBLOCKLOS)

    self.JetProp1 = self.PackProp1
    self.JetProp2 = self.PackProp2
...
local resetFlyAnimTime = 0
function ENT:CustomOnThink()
    self:SoldierThink()
    local wep = self:GetActiveWeapon()
    local curTime = CurTime()
    if IsValid(wep) then
        if self.WeaponSpread != self.Soldier_WeaponSpread && wep:GetClass() != "weapon_vj_combine_sniper_rifle_z" then
            self.WeaponSpread = self.Soldier_WeaponSpread
            --print("normal spread")
        end
    end

    if self.NextLeapTime <= curTime then
        timer.Simple( 1, function()
            if IsValid(self) then 
                --self:VJ_ACT_PLAYACTIVITY("jump_holding_jump"--[["jump_holding_glide"]], false, 2, false)
                local moveCheck = VJ_PICK(self:VJ_CheckAllFourSides(700, true, "1111"))
                if moveCheck then
                    self:LeapAttackVelocityCode(moveCheck) 
                    resetFlyAnimTime = CurTime() + 0.5
                end
            end
        end) 
        self.NextLeapTime = curTime + math.random( 2, 10 )
    end

    if resetFlyAnimTime < CurTime() then
        if self:IsOnGround() == false and wasFlying == false then
            resetFlyAnimTime = CurTime() + 1
            self:VJ_ACT_PLAYACTIVITY("jump_holding_jump",true,0.5,true) --self:SetSequence("jump_holding_jump")
            wasFlying = true
        elseif self:IsOnGround() and wasFlying == true then
            resetFlyAnimTime = CurTime() + 2
            self:VJ_ACT_PLAYACTIVITY("jump_holding_land",true,0.5,true) --self:SetSequence("jump_holding_land")
            self:StopSound( "weapons/flaregun/burn.wav" )
            wasFlying = false
        end
    end
end
...
---------------------------------------------------------------------------------------------------------------------------------------------
function ENT:LeapAttackVelocityCode(pos)
	local ene = self:GetEnemy()
    local moveCheck = pos --VJ_PICK(self:VJ_CheckAllFourSides(700, true, "1111"))
	if !IsValid(ene) then return end
	self:SetGroundEntity(NULL)
    self:EmitSound("weapons/flaregun/burn.wav",90,math.random(85, 115))
	if self.LeapAttackAnimationFaceEnemy == true then self:FaceCertainEntity(ene, true) end
	self.LeapAttackHasJumped = true
    self:AddJetTrail()
    if moveCheck then
	    self:SetLocalVelocity(((moveCheck) - (self:GetPos() + self:OBBCenter())):GetNormal()*math.random(200, 500) + self:GetForward()*self.LeapAttackVelocityForward + self:GetUp()*self.LeapAttackVelocityUp + self:GetRight()*self.LeapAttackVelocityRight)
    end
end

function ENT:AddJetTrail()
    local sprite1 = ents.Create( "env_sprite" )
	sprite1:SetKeyValue( "rendercolor","206 187 168" )
	sprite1:SetKeyValue( "GlowProxySize","2.0" )
	sprite1:SetKeyValue( "HDRColorScale","1.0" )
	sprite1:SetKeyValue( "renderfx","14" )
	sprite1:SetKeyValue( "rendermode","3" )
	sprite1:SetKeyValue( "renderamt","240" )
	sprite1:SetKeyValue( "disablereceiveshadows","0" )
	sprite1:SetKeyValue( "mindxlevel","0" )
	sprite1:SetKeyValue( "maxdxlevel","0" )
	sprite1:SetKeyValue( "framerate","10.0" )
	sprite1:SetKeyValue( "model","sprites/blueflare1.spr" )
	sprite1:SetKeyValue( "spawnflags","0" )
	sprite1:SetKeyValue( "scale","0.80" )
	sprite1:SetPos( self:GetPos() + self:OBBCenter() - (self:GetForward()*9+self:GetRight()*5+self:GetUp()*9) )
    sprite1:SetParent(self,2)
	sprite1:Spawn()

	local sprite2 = ents.Create( "env_sprite" )
	sprite2:SetKeyValue( "rendercolor","206 187 168" )
	sprite2:SetKeyValue( "GlowProxySize","2.0" )
	sprite2:SetKeyValue( "HDRColorScale","1.0" )
	sprite2:SetKeyValue( "renderfx","14" )
	sprite2:SetKeyValue( "rendermode","3" )
	sprite2:SetKeyValue( "renderamt","240" )
	sprite2:SetKeyValue( "disablereceiveshadows","0" )
	sprite2:SetKeyValue( "mindxlevel","0" )
	sprite2:SetKeyValue( "maxdxlevel","0" )
	sprite2:SetKeyValue( "framerate","10.0" )
	sprite2:SetKeyValue( "model","sprites/blueflare1.spr" )
	sprite2:SetKeyValue( "spawnflags","0" )
	sprite2:SetKeyValue( "scale","0.80" )
	sprite2:SetPos( self:GetPos() + self:OBBCenter() - (self:GetForward()*11+self:GetRight()*-5+self:GetUp()*9))
    sprite2:SetParent(self,2)
	sprite2:Spawn()

    util.SpriteTrail(sprite1, 0, Color(206, 187, 168, 150), false, 20, 50, 0.5, 5 / ((2 + 10) * 0.5), "trails/smoke.vmt")
    util.SpriteTrail(sprite2, 0, Color(206, 187, 168, 150), false, 20, 50, 0.5, 5 / ((2 + 10) * 0.5), "trails/smoke.vmt")
...
function ENT:CustomOnDeath_AfterCorpseSpawned(dmginfo, hitgroup, corpseEnt) 
    self:StopSound( "weapons/flaregun/burn.wav" )
    self.JetProp1:SetMoveParent( self.Corpse )
    self.JetProp2:SetMoveParent( self.Corpse )
