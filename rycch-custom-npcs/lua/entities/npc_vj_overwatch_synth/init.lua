...
ENT.Model = {"models/cmb_synth_elite.mdl"}
ENT.StartHealth = 150
...
function ENT:VJ_CheckAllEightSides(checkDist, returnPos, sides)
	checkDist = checkDist or 200
	sides = sides or str1111
	local result = returnPos == true and {} or {Forward=false, Backward=false, Right=false, Left=false, ForwardRight=false, BackwardRight=false, ForwardLeft=false, BackwardLeft=false}
	local i = 0
    local return_pos = self:GetRight()*200
	local myPos = self:GetPos()
	local myPosCentered = myPos + self:OBBCenter()
	local myForward = self:GetForward()
	local myRight = self:GetRight()
    local myForwardRight = self:GetForward() + self:GetRight()
    local myBackwardRight = -1*self:GetForward() + self:GetRight()
	local positions = { -- Set the positions that we need to check
        ...
        string_sub(sides, 5, 5) == str1 and myForwardRight or 0,
		string_sub(sides, 6, 6) == str1 and -myForwardRight or 0,
		string_sub(sides, 7, 7) == str1 and myBackwardRight or 0,
		string_sub(sides, 8, 8) == str1 and -myBackwardRight or 0
	}
	for _, v in ipairs(positions) do
		...
		if myPos:Distance(hitPos) >= checkDist then
			...
			elseif i == 4 then
				result.Left = true
            elseif i == 5 then
				result.ForwardRight = true
            elseif i == 6 then
				result.BackwardRight = true
            elseif i == 7 then
				result.ForwardLeft = true
            elseif i == 8 then
				result.BackwardLeft = true
			end
		end
	end
	return result
