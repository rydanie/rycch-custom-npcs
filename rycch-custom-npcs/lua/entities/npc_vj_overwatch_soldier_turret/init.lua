...
ENT.Model = {"models/jq/hlvr/characters/combine/combine_captain/combine_captain_hlvr_npc.mdl"}
ENT.StartHealth = 100
...
function ENT:CustomOnInitialize()
    ...
    local turretchance = 100 --GetConVar("vj_zippycombines_soldier_turretchance"):GetInt()
...
function ENT:CustomOnThink()
    ...
    if self.CanDropCover == true and self.DeployingTurret == false then
        self:DeployCover()
...
function ENT:DeployCover()
    ...
    timer.Simple(manhack_deploy_time, function() if IsValid(self) then
        ...
        local manhack = ents.Create("cover_deployer")
        --local _shieldDeployAngleYaw = self:GetAimVector().Normal:Angle().yaw
        ...
        local shieldDeployerPhys = manhack.phys -- GET THIS AFTER SPAWNING AND ACTIVATING NEXT TIME DICK ED
        manhack:Activate()
        if(manhack:GetPhysicsObject():IsValid() ) then 
            shieldDeployerPhys:SetVelocityInstantaneous(self:GetAimVector() * 20)  
        end
    end end)
