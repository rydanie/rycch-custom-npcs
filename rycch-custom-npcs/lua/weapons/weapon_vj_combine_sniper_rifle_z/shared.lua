...
SWEP.Primary.TracerType = "AirboatGunHeavyTracer"
...
function SWEP:SetLaser(skin_state)
	if skin_state == true then
		self:SetNWBool( "Laser_Setting", true )
	else
		self:SetNWBool( "Laser_Setting", false )
	end
	...
	if IsValid(owner) then
		if owner:IsPlayer() then
			...
			if IsValid(enemy) && owner:Visible(enemy) && self:Clip1() > 0 then
				...
				if curTime >= rest_time and self:NPCAbleToShoot() then 
					if laser_ready == true then
						self:SetNWBool( "Laser_Setting", true )
						rest_time = curTime + 2
					end
					laser_ready = true
...
function SWEP:CustomOnDrawWorldModel()
	...
	local laserSetting = self:GetNWBool( "Laser_Setting")
	...
	if IdealPos != laserstartpos and laserSetting == true then
