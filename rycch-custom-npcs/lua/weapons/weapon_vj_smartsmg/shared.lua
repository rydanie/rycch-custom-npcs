...
SWEP.NPC_TimeUntilFireExtraTimers = {0.2, 0.3} -- Extra timers, which will make the gun fire again! | The seconds are counted after the self.NPC_TimeUntilFire!
	-- Main Settings ---------------------------------------------------------------------------------------------------------------------------------------------
SWEP.ViewModel = "models/weapons/c_irifle.mdl"
SWEP.WorldModel = "models/weapons/cup_weapons/w_smg1.mdl"
SWEP.HoldType = "smg"
SWEP.Spawnable = true
...
function SWEP:CustomOnPrimaryAttack_BeforeShoot()
	...
	local shoot_angle = self.Owner:GetAimVector() --Vector(0.09,0.09,0.09)

	local shot = ents.Create( "obj_vj_smart_bullet" )
	local shoot_pos   = self.Owner:GetPos() + self.Owner:GetRight() * 5 + self.Owner:GetUp() * 47 + shoot_angle --* 20
	if ( IsValid( shot ) ) then
		shot:SetPos( shoot_pos )
		shot:SetAngles( shoot_angle:Angle() )
		shot:SetOwner( self.Owner )
		shot.MyEnemy = self:GetOwner():GetEnemy()
		shot:Spawn()
	end
end
---------------------------------------------------------------------------------------------------------------------------------------------
