...
ENT.StartHealth = 250
ENT.Model = {"models/theparrygod/transition_shotgunner_npc.mdl"}
ENT.wep = "weapon_vj_Heavy_Shotty"
...
ENT.Weapon_FiringDistanceFar = 350
...
ENT.NextMoveRandomlyWhenShootingTime1 = 0.2 -- How much time until it can move randomly when shooting? | First number in math.random
...
ENT.AnimationPlaybackRate = 1.0 -- Controls the playback rate of all the animations

ENT.HasShootWhileMoving = true
...
--ENT.AnimTbl_WeaponAim = {ACT_RUN_AIM}
ENT.AnimTbl_TakingCover = {ACT_RUN_AIM} -- The animation it plays when hiding in a covered position
ENT.AnimTbl_MoveToCover = {ACT_RUN_AIM} -- The animation it plays when moving to a covered position
...
ENT.AnimTbl_Run = {ACT_RUN_AIM} -- Set the running animations | Put multiple to let the base pick a random animation when it moves
...
function ENT:SoldierThink() 
    if checkTime < CurTime() then
        ...
        checkTime = CurTime() + math.random(0.7, 1.5)
    end
end
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
