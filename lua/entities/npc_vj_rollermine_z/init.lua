...
function ENT:CustomOnInitialize()
    ...
    if self.HasExplosionAttack == true then
        self.SpikeModel:SetColor( Color(255, 0, 0, 255) )
        self.SpikeModel:SetModelScale( 0.6, 0 )
    end
        ...
        if self.HasExplosionAttack then
            ...
            if self.VJ_IsBeingControlled then
                ...
                if enemydist < self.ExplodeDist then
                    self.IsExploding = true
                    self:SetVelocity(Vector(0,0,190))
                    self:EmitSound("npc/roller/mine/rmine_predetonate.wav", 90, math.random(90, 110))
                    timer.Simple(0.35, function() if IsValid(self) then self:TakeDamage(10000000, self, self) end end)
                end
            end
