...
ENT.Model = {"models/dpfilms/metropolice/c08cop.mdl"}
ENT.StartHealth = 50
...
ENT.AnimTbl_TakingCover = {ACT_WALK} -- The animation it plays when hiding in a covered position
ENT.AnimTbl_MoveToCover = {ACT_WALK} -- The animation it plays when moving to a covered position
...
ENT.wep = "weapon_vj_stunstick_z"

-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
function ENT:SoldierInit()
    self:Give(self.wep)

    local deploypos = self:GetPos() + self:GetForward() * 25 + self:GetUp() * 19 + self:GetRight() * -16
    local deploypos2 = self:GetPos() + self:GetForward() * -15.2 + self:GetUp() * 14 + self:GetRight() * 0
    self.shield = ents.Create("cup_shield")
    self.shield:SetPos( deploypos )
    self.shield:SetParent(self, 3)
    self.shield:SetAngles( self:GetAngles() + Angle(20,200,5) )
    ...
    self.shield3:SetParent(self, 3)
    self.shield3:SetNoDraw( true )
    self.shield3:SetAngles( self:GetAngles() + Angle(5,0,0) )
    self.shield3:SetCollisionGroup(COLLISION_GROUP_WEAPON)
    self.shield3:SetOwner( self )
    self.shield3:Spawn()
