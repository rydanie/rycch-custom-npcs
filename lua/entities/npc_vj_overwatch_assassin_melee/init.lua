...
ENT.MaxJumpLegalDistance = VJ_Set(600, 850) -- The max distance the NPC can jump (Usually from one node to another) | ( UP, DOWN )
ENT.Weapon_FiringDistanceFar = 35
...
ENT.wep = "weapon_vj_assassin_pistols_melee"
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
function ENT:SoldierThink() 
    ...
    if GetConVar("ai_disabled"):GetInt() == 0 then
        ...
        if  self.LatestEnemyDistance >= cloak_dist then
